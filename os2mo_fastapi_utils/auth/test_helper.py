...
def ensure_endpoints_depend_on_oidc_auth_function(
    ...
    routes = filter(lambda _route: isinstance(route, APIRoute), all_routes)
    # Only check endpoints not in the NO_AUTH_ENDPOINTS list
    routes = filter(lambda _route: route.path not in no_auth_endpoints, routes)
    for route in routes:
...
def ensure_no_auth_endpoints_do_not_depend_on_auth_function(
    ...
    no_auth_routes = filter(lambda _route: route.path in no_auth_endpoints, all_routes)
